...
from difflib import SequenceMatcher   # NEW: brand spoof similarity check
...
class FeatureExtraction:
    ...
    def __init__(self, url):
        ...
        # ---- IMPORTANT: URL ko normalize karna ----
        # agar user "paypa1.com" type kare (scheme nahi diya),
        # to hum "http://paypa1.com" bana denge
        fixed_url = self.url
        if "://" not in fixed_url:
            fixed_url = "http://" + fixed_url

        try:
            self.response = requests.get(fixed_url, timeout=5)
            self.soup = BeautifulSoup(self.response.text, "html.parser")
        except:
            pass

        try:
            self.urlparse = urlparse(fixed_url)
            # domain hamesha lowercase me
        ...
        # ---- yahan se aage wahi purani 30 feature calls ----
    ...
    def StatsReport(self):
        try:
            ...
            ip_match = re.search(
                ...
                '181\.174\.165\.13|46\.242\.145\.103|121\.50\.168\.40|83\.125\.22\.219|46\.242\.145\.98|'
                ...
                '209\.99\.17\.27|216\.38\.62\.18|104\.130\.124\.96|47\.89\.58\.141|78\.46\.211\.158|'
                '54\.86\.225\.156|54\.82\.156\.19|37\.157\.192\.102|204\.11\.56\.48|110\.34\.231\.42',
                ip_address
            )
    ...
    # ------------ EXTRA HELPERS (ML ke bahar use honge) ------------

    def brand_spoof_flag(self):
        """
        Famous brands ka spoof detect karega.
        Example: 'paypa1' vs 'paypal' -> -1 (suspicious)
        """
        try:
            host = self.domain.split(".")[0].lower()
        except:
            return 1

        brands = [
            "paypal", "google", "facebook", "instagram", "amazon",
            "whatsapp", "microsoft", "netflix", "apple",
            "flipkart", "phonepe", "paytm",
            "sbi", "hdfc", "icici", "axis"
        ]

        for b in brands:
            ratio = SequenceMatcher(None, host, b).ratio()
            # 0.8 similarity & not exact same => spoofing
            if ratio >= 0.8 and host != b:
                return -1

        return 1


    def domain_exists_flag(self):
        """
        DNS / IP lookup se check karega ki domain exist karta hai ya nahi.
        Resolve nahi ho raha -> -1
        """
        try:
